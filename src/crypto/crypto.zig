...
// Import primitives types directly
const Hex = @import("../primitives/hex.zig").Hex;
...
pub const Signature = struct {
    ...
    pub fn to_hex(self: Signature) [132]u8 {
        ...
        return Hex.encodeFixed(65, bytes);
    }

    pub fn from_hex(hex_str: []const u8) !Signature {
        const bytes = try Hex.decodeFixed(65, hex_str);
...
test "address derivation" {
    ...
    try testing.expect(!std.mem.eql(u8, &address.bytes, &zero_address));
}
...
test "invalid signature rejection" {
    ...
    const wrong_address = Address{ .bytes = [_]u8{0xFF} ** 20 };
...
test "derive address from private key" {
    ...
    try testing.expectEqualSlices(u8, &address.bytes, &address2.bytes);
}
