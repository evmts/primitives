...
## Generic Frame Handlers

### Handler Pattern

EVM opcode handlers are implemented using a generic pattern that operates on any Frame type meeting specific interface requirements. This enables custom Frame implementations with different storage backends, word types, and optimization strategies.
...
### Frame Interface Requirements

Custom Frame implementations must provide:

```zig
pub const CustomFrame = struct {
    // Required types
    pub const Error = error{ OutOfGas, StackUnderflow, ... };
    pub const WordType = u256; // or u128, u512, etc.
    pub const UintN = U256; // Arbitrary precision type
    ...
    pub fn beforeInstruction(
        self: *@This(),
        opcode: Dispatch.UnifiedOpcode,
        cursor: [*]const Dispatch.Item,
    ) void;

    pub fn afterInstruction(
        self: *@This(),
        opcode: Dispatch.UnifiedOpcode,
...
const DefaultFrame = Frame(default_config);
```
...
```zig
const ArithmeticHandlers = Handlers(FrameType);

// ADD (0x01): a + b
pub fn add(frame: *FrameType, cursor: [*]const Dispatch.Item) Error!noreturn;

// MUL (0x02): a * b
pub fn mul(frame: *FrameType, cursor: [*]const Dispatch.Item) Error!noreturn;

// SUB (0x03): a - b
pub fn sub(frame: *FrameType, cursor: [*]const Dispatch.Item) Error!noreturn;

// DIV (0x04): a / b (returns 0 if b == 0)
pub fn div(frame: *FrameType, cursor: [*]const Dispatch.Item) Error!noreturn;

// SDIV (0x05): signed a / b
pub fn sdiv(frame: *FrameType, cursor: [*]const Dispatch.Item) Error!noreturn;

// MOD (0x06): a % b (returns 0 if b == 0)
pub fn mod(frame: *FrameType, cursor: [*]const Dispatch.Item) Error!noreturn;

// SMOD (0x07): signed a % b
pub fn smod(frame: *FrameType, cursor: [*]const Dispatch.Item) Error!noreturn;

// ADDMOD (0x08): (a + b) % N
pub fn addmod(frame: *FrameType, cursor: [*]const Dispatch.Item) Error!noreturn;

// MULMOD (0x09): (a * b) % N
pub fn mulmod(frame: *FrameType, cursor: [*]const Dispatch.Item) Error!noreturn;

// EXP (0x0a): a ** b
pub fn exp(frame: *FrameType, cursor: [*]const Dispatch.Item) Error!noreturn;

// SIGNEXTEND (0x0b): sign extend value
pub fn signextend(frame: *FrameType, cursor: [*]const Dispatch.Item) Error!noreturn;
```
...
const FrameConfig = @import("frame").FrameConfig;
const MemoryDatabase = @import("storage").MemoryDatabase;
...
const DefaultFrame = Frame(config);
const Handlers = DefaultFrame.ArithmeticHandlers;

var frame = try DefaultFrame.init(
    allocator,
    gas_limit,
    database,
    contract_address,
...
_ = try Handlers.add(&frame, dispatch.cursor);
```

#### Custom Frame with Different Word Type

```zig
const u128_config = FrameConfig{
    .stack_size = 512,
    .WordType = u128, // 128-bit words
    .DatabaseType = CustomDatabase,
};

const U128Frame = Frame(u128_config);

var frame = try U128Frame.init(...);
const Handlers = U128Frame.ArithmeticHandlers;

// All handlers work with u128 operations
_ = try Handlers.mul(&frame, dispatch.cursor);
```

#### Custom Frame with Tracing

```zig
const TracedFrame = struct {
    base_frame: DefaultFrame,
    trace: std.ArrayList(TraceEvent),

    pub const Error = DefaultFrame.Error;
    pub const WordType = DefaultFrame.WordType;
    pub const UintN = DefaultFrame.UintN;
    pub const Dispatch = DefaultFrame.Dispatch;
    ...
    pub fn beforeInstruction(
        ...
        self.trace.append(.{
            .opcode = opcode,
            .gas_before = self.base_frame.gas_remaining,
        }) catch {};

        self.base_frame.beforeInstruction(opcode, cursor);
    }
    ...
    pub fn afterInstruction(
        ...
        // Log after execution
        self.trace.append(.{
            .gas_after = self.base_frame.gas_remaining,
        }) catch {};

        self.base_frame.afterInstruction(opcode, next_handler, next_cursor);
    }

    // Delegate stack operations
    pub const stack = @field(@This(), "base_frame").stack;
};

const TracedHandlers = Handlers(TracedFrame);
```

#### Custom Optimized Frame

```zig
// Frame optimized for specific use case
const OptimizedConfig = FrameConfig{
    .stack_size = 256, // Smaller stack
    .WordType = u256,
    .max_bytecode_size = 4096, // Smaller contracts only
    .memory_initial_capacity = 1024, // Less memory
    .DatabaseType = ReadOnlyDatabase, // No state changes
};

const OptimizedFrame = Frame(OptimizedConfig);

// All handlers automatically use optimized configuration
const OptimizedHandlers = OptimizedFrame.ArithmeticHandlers;
```
...
All arithmetic operations use wrapping arithmetic to match EVM semantics:
...
Division and modulo operations return 0 when divisor is zero:

```zig
// DIV: returns 0 if divisor is 0
if (divisor == 0) return 0;

// SDIV: returns 0 if divisor is 0
if (divisor == 0) return 0;
```

#### Signed Arithmetic

Signed operations use branchless two's complement arithmetic:

```zig
// SDIV: branchless signed division
...
ADDMOD and MULMOD use arbitrary precision to prevent overflow:

```zig
// ADDMOD: (a + b) % N with overflow handling
...
// MULMOD: (a * b) % N using double-width arithmetic
...
Dynamic gas costs are computed within handlers:

```zig
// EXP: dynamic gas based on exponent size
var exp_bytes: u32 = 0;
if (exponent > 0) {
...
- **Stack operations**: O(1) for push/pop with bounds checking
- **Arithmetic**: Branch-free implementations for consistent timing
- **Modular arithmetic**: Uses double-width types to prevent overflow
- **Gas metering**: Negative gas pattern for single-branch OOG detection
- **Tail calls**: Used for handler chaining to prevent stack buildup
